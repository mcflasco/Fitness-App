...
//		mike.setName("Cameron");
		...
		//Hibernate setup
				Configuration con = new Configuration().configure().addAnnotatedClass(Employee.class).addAnnotatedClass(Alien.class).addAnnotatedClass(Build.class); 	
...
//		
			...
			int setTwo = input.nextInt();
				
			System.out.println("Heavy: ");
			int setThree = input.nextInt();
...
//					repsOne, repsTwo, repsThree);
			...
			newBuild.setId(3);
			newBuild.setSetOne(setOne);
			newBuild.setSetTwo(setTwo);
			newBuild.setSetThree(setThree);
			session.save(newBuild);
			tx.commit();
		}
